...
let semicolon_dwim () =
  ...
  | Some (start, end_) ->
    Background.don't_wait_for [%here] (fun () ->
        (* Since this is in the background, assume the current buffer might
               change across any [Deferred.bind] or [Deferred.map]. *)
        ...
        Current_buffer.set_temporarily Sync buffer ~f:(fun () ->
            ...
            | Some prefix ->
              ...
              Point.delete_forward_char_exn
                (Position.diff (Marker.position point |> Option.value_exn) (Point.get ()));
              insert_before_markers " in";
              Point.goto_char start;
              Point.insert
                (match prefix with
                | "unit" -> "let%bind () = "
                | _ -> [%string "let%bind (_ : %{prefix}) = "]);
...
let () =
  defun_nullary_nil
    ...
    (fun () ->
      Point.insert ";";
      if Minor_mode.is_enabled Tree_sitter.mode then semicolon_dwim ())
;;
